...
class MoERunner(MoERunnerInterface):
    ...
    def _apply_quant_method(
        ...
        """
        # NOTE: the try/except below is a kit-local robustness fix (not
        # upstream). SharedExperts._output[idx] is produced by the
        # _maybe_apply_shared_experts calls below and consumed ONLY by the
        # `self._shared_experts.output` property at the end of this method
        # (which clears the slot). If any step between a produce and the
        # consume raises (routine during warmup/capture/inductor recompile),
        # the slot stays dirty and the next step's SharedExperts.forward
        # hits `assert self._output[idx] is None` (shared_experts.py),
        # permanently poisoning the layer. Clear the slot on the exception
        # path so an aborted step cannot leak state into the next one.
        try:
            ...
            # Aborted step: clear any shared-expert output produced before
            # the exception so the layer slot is not poisoned (see NOTE
            # above). BaseException also covers KeyboardInterrupt during
            # CUDA-graph capture.
            if self._shared_experts is not None:
                self._shared_experts._output[
                    self._shared_experts._output_idx
                ] = None
            raise
