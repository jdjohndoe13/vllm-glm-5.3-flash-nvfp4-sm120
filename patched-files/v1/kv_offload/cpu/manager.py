...
class CPUOffloadingManager(OffloadingManager):
    ...
    def prepare_store(
        ...
        num_blocks_to_evict = len(keys_to_store) - free_before
        ...
        # CPU-TIER-EVICT diagnostics 2026-09-14: one INFO line per evicting
        # prepare_store call (not per key). Snapshot taken here, i.e.
        # allocated is pre-allocation (eviction never changes it) and
        # evictable_len is post-eviction. No rate guard: evicting calls are
    ...
    def get_stats(self) -> OffloadingConnectorStats | None:
        ...
        # num_used above counts ONLY pinned blocks (write-pending + in-flight
        # loads); idle stored-ready chunks live in _num_evictable_cache_blocks
        # and are invisible to that gauge, so a full idle tier reads ~0%.
        # These gauges expose the raw counters instead:
        # stored occupancy = allocated - free_list_len (evictable + pinned).
        stats.set_gauge(
            CPUOffloadingMetrics.CPU_ALLOCATED_BLOCKS, self._num_allocated_blocks
        )
        stats.set_gauge(CPUOffloadingMetrics.CPU_FREE_LIST_LEN, len(self._free_list))
        stats.set_gauge(
