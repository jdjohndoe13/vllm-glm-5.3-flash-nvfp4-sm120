...
class CPUOffloadingSpec(OffloadingSpec):
    ...
    def build_metric_definitions(
        ...
        definitions: dict[str, OffloadingMetricMetadata] = {
            ...
            # + cumulative eviction counter. Scheduler-side manager snapshot:
            # tier-wide (one manager per engine), no per-rank dimension.
            CPUOffloadingMetrics.CPU_ALLOCATED_BLOCKS: OffloadingGaugeMetadata(
                documentation=(
                    "CPU tier block pool: blocks currently allocated (fresh "
                    "or reused); equals free-list blocks + blocks holding "
                    "stored data (evictable or pinned)."
                ),
            ),
            CPUOffloadingMetrics.CPU_FREE_LIST_LEN: OffloadingGaugeMetadata(
                documentation=(
                    "CPU tier block pool: freed block ids currently in the "
                    "free list, reusable without eviction."
                ),
            ),
            CPUOffloadingMetrics.CPU_EVICTABLE_LEN: OffloadingGaugeMetadata(
                documentation=(
                    "CPU tier block pool: blocks holding stored-but-idle "
                    "data (ref_cnt 0), i.e. LRU eviction candidates."
                ),
            ),
            CPUOffloadingMetrics.CPU_EVICTED_TOTAL: OffloadingGaugeMetadata(
                documentation=(
                    "Cumulative CPU tier blocks evicted from the tier by "
                    "prepare_store since engine boot (LRU churn signal)."
                ),
            ),
        }
