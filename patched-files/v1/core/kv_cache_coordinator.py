...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def find_longest_cache_hit(
        ...
        # A6 2026-09-18c/d: identical per-step repeats -> DEBUG (change-detect).
        # Fail-safe: any state error degrades to the original INFO behavior.
        try:
            _apc_sig = (len(block_hashes), max_cache_hit_length, hit_length,
                        tuple(hit_length_by_group),
                        num_uncached_common_prefix_tokens)
            _apc_prev = getattr(self, "_last_apc_sig", None)
            self._last_apc_sig = _apc_sig
            _apc_dedup = _apc_prev == _apc_sig
        except Exception:
            _apc_dedup = False
        (logger.debug if _apc_dedup else logger.info
         if hit_length else logger.debug)(
