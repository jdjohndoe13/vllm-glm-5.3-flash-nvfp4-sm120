...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # ESC211 2026-09-21: connector escape hook. When the KV connector is
        # wedged answering (0, False) for a giant identical hit (escape mode,
        # see the connector's is_stale_hit_escape / KV-LOOKUP SERVERKILL),
        # skipping the full-sequence admission gate for that request lets the
        # chunked recompute path schedule it instead of waiting for the pool
        # to free whole-sequence room. Disable the scheduler-side consumption
        # with VLLM_KV_OFFLOAD_ESCAPE_RELAX_FULL_FIT=0.
        self._stale_escape_relax_full_fit = (
            os.environ.get("VLLM_KV_OFFLOAD_ESCAPE_RELAX_FULL_FIT", "1")
            .strip()
            .lower()
            not in ("0", "false", "off")
        )
        self._connector_escape_check = (
            getattr(self.connector, "is_stale_hit_escape", None)
            if self.connector is not None
            else None
        )
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                # ESC211 2026-09-21: relax the full-sequence admission gate
                # only while the connector is in escape mode for this
                # request (giant identical-hit wedge, see
                # VLLM_KV_OFFLOAD_ESCAPE_RELAX_FULL_FIT). Sync loads keep
                # their normal full-fit admission.
                _stale_escape = (
                    self._stale_escape_relax_full_fit
                    and self._connector_escape_check is not None
                    and not load_kv_async
                    and self._connector_escape_check(request)
                )
                new_blocks = self.kv_cache_manager.allocate_slots(
                    request,
                    num_new_tokens,
                    ...
                    full_sequence_must_fit=(
                        self.scheduler_reserve_full_isl and not _stale_escape
                    ),
